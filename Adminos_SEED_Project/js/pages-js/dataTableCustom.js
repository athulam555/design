$(document).ready(function() {
    ...
    $('<button class="btn btn-primary  mb-3 mr-1">sum of age in all rows</button>')
        .prependTo('.dt-plugin-buttons')
        .on('click', function() {
            alert('Column sum is: ' + table.column(3).data().sum());
        });

    $('<button class="btn btn-primary m-r-10 mb-3 mr-1">sum of  age of visible rows</button>')
        .prependTo('.dt-plugin-buttons')
        .on('click', function() {
    ...
    $('#javascript-source-table').DataTable({
        data: dataSet,
        columns: [
            { title: "Name" },
            { title: "Position" },
    ...
    $('#alt-pagination-dt').DataTable({
        "pagingType": "full_numbers"
    });

    $('#scroll-vertical-dt').DataTable({
        "scrollY": "200px",
        "scrollCollapse": true,
        "paging": false
    });

    $('#scroll-vertical-dynamic').DataTable({
        scrollY: '50vh',
        scrollCollapse: true,
        paging: false
    });

    $('#language-dt').DataTable({
        "language": {
            "decimal": ",",
            "thousands": "."
        }
